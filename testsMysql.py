...
def connexion():
    return mysql.connector.connect(host="localhost", user="root", password="", database="sirene")
            ...
            if i==0:
                ...
                if(i==9):
                    if(str(row[i]) == "nan"):
                        ...
                        requete += 'CAST(\'' + splitted[0] + ' ' + splitted[1] +'\' AS DateTime), '
                elif(i==4 or i==6 or i==11 or i==13 or i==24 or i==27 or i==40):
                    if(i==4 or i==40):
                        if(str(row[i]) == "nan"):
                            requete += 'NULL, '
                        else:
                            requete += str(row[i]).replace('/', '-')+', '
                    else:
                        if(str(row[i]) == "nan"):
                            requete += 'NULL, '
                        else:
            ...
            if i==0:
                ...
                if(i==9):
                    if(str(row[i]) == "nan"):
                        ...
                        splitted = str(row[i]).split('+')
                        splitted = splitted[0].split('T')
                        requete_etablissement += 'CAST(\'' + splitted[0] + ' ' + splitted[1] +'\' AS DateTime), '
                elif(i==4 or i==6 or i==11 or i==13 or i==24 or i==27 or i==40):
                    if(i==4 or i==40):
                        if(str(row[i]) == "nan"):
                            requete_etablissement += 'NULL, '
                        else:
                            requete_etablissement += str(row[i]).replace('/', '-')+', '
                    else:
                        if(str(row[i]) == "nan"):
                            requete_etablissement += 'NULL, '
                        else:
                ...
                if(i==64):
                    if(str(row[i]) == "nan"):
                        ...
                        splitted = str(row[i]).split('+')
                        splitted = splitted[0].split('T')
                        requete_unitelegale += 'CAST(\'' + splitted[0] + ' ' + splitted[1] +'\' AS DateTime), '
                elif(i==51 or i==62 or i==65):
                    if(i==51):
                        if(str(row[i]) == "nan"):
                            requete_unitelegale += 'NULL, '
                        else:
                            requete_unitelegale += str(row[i]).replace('/', '-')+', '
                    else:
                        if(str(row[i]) == "nan"):
                            requete_unitelegale += 'NULL, '
                        else:
...
def plot_graphiques_insert():
    afficher_graphique(array = tempsInsertionsTablesNonFragmentees, xlabel = 'numéro de l\'insertion', ylabel = 'temps en secondes', title = 'Temps en secondes de chaque insertion (MySQL, table non fragmentée)')
    afficher_graphique(array = tempsInsertionsTablesFragmenteesEnsembles, xlabel = 'numéro de l\'insertion', ylabel = 'temps en secondes', title = 'Temps en secondes de chaque insertion (MySQL, tables fragmentées ensembles)')
    afficher_graphique(array = tempsInsertionsTableFragmenteeEtablissement, xlabel = 'numéro de l\'insertion', ylabel = 'temps en secondes', title = 'Temps en secondes de chaque insertion (MySQL, table fragmentée ETABLISSEMENT)')
    afficher_graphique(array = tempsInsertionsTableFragmenteeUniteLegale, xlabel = 'numéro de l\'insertion', ylabel = 'temps en secondes', title = 'Temps en secondes de chaque insertion (MySQL, table fragmentée UNITELEGALE)')
    afficher_graphique(array = tempsInsertionsTableFragmenteeEntreprise, xlabel = 'numéro de l\'insertion', ylabel = 'temps en secondes', title = 'Temps en secondes de chaque insertion (MySQL, table fragmentée ENTREPRISE)')

db = connexion()
cursor = db.cursor(buffered=True)

exec_fichier(cursor, 'creation_tables_mysql.sql')
